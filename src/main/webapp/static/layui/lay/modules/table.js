...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
	...
	F.prototype.config = {
		...
		"object" == typeof a.defaultToolbar && layui.each(a.defaultToolbar, function(e, t) {
			var i = r[t];
			if (i.title == '导出当前' || i.title == '导出全部') {
				i && d.push('<button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="' + i.layEvent +
					'"><i class="layui-icon ' + i.icon + '"></i>' + i.title + '</button>')
			} else {
		...
		a.layTool.on("click", "*[lay-event]", function(e) {
			...
			switch (layui.stope(e), R.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
				...
				case "LAYTABLE_EXPORT":
					...
					break;
				case "LAYTABLE_EXPORT_ALL":
					r.ie ? l.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {
						tips: 3
					}) : s({
						list: function() {
							return ['<li data-type="csv" id="csv_exoprt_all">导出到 Csv 文件</li>',
								'<li data-type="xls" id="xls_exoprt_all">导出到 Excel 文件</li>'
							].join("")
						}(),
						done: function(e, i) {
							i.on("click", function() {
